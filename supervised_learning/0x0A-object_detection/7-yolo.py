...
class Yolo:
    ...
    def predict(self, folder_path):
        ...
        for i in range(pimages.shape[0]):
            current = [out[i] for out in outputs]

            boxes, box_confidences, box_class_probs = \
                self.process_outputs(current, image_shapes[i])

            filtered_boxes, box_classes, box_scores = \
                self.filter_boxes(boxes, box_confidences, box_class_probs)

            box_predictions, predict_box_classes, predict_box_scores = \
